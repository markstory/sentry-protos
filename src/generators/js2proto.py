...
class NamedSchema:
    ...
    def unpack(
        ...
        if (
            self.schema.anyOf is not None
            and len(self.schema.anyOf) == 2
            and self.schema.anyOf[1].type == DataType.NULL
        ):
        ...
        if self.schema.anyOf and len(self.schema.anyOf) == 1:
            return dataclasses.replace(
                self, schema=self.resolve(self.schema.anyOf[0])
            ).unpack()
        ...
        if self.schema.anyOf:
            ...
            for part in self.schema.anyOf:
                if isinstance(part, Reference):
                    name = part.ref.split("/")[-1]
                    parts.append(
                        NamedSchema(
                            name=name,
                            schema=self.resolve(part),
        ...
        if self.schema.type is not None and self.schema.type == DataType.ARRAY:
            if self.schema.prefixItems:
                # doesn't support mixing the two
                if self.schema.items:
                    return _list
                result = NamedStructureType(
                    definition_dependencies=[],
                    properties={},
                    definitions=self.definitions,
                )
                for i, item in enumerate(self.schema.prefixItems):
                    named = result.properties[f"a{i}"] = NamedSchema(
                        name="",
                        schema=self.resolve(item),
                        definitions=self.definitions,
                    )
                    if isinstance(item, Reference):
                        named.name = item.ref.split("/")[-1]
                    else:
                        named.name = f"{self.name}{i}"
                        result.definition_dependencies.append(f"a{i}")
                return result
        ...
        if (
            ...
            and not self.schema.properties
            and isinstance(self.schema.additionalProperties, (Reference, Schema))
        ):
        ...
        if self.schema.const is not None:
            if isinstance(self.schema.const, int):
                return ScalarType(
                    DataType.INTEGER,
                    extension=f"(options.v1.const).int = {self.schema.const!r}",
                )
            if isinstance(self.schema.const, str):
                return ScalarType(
                    DataType.STRING,
                    extension=f"(options.v1.const).string = {self.schema.const!r}",
                )
...
class TopLevelSchema:
    ...
    def schemas(self) -> "Iterator[NamedSchema]":
        definitions = {
            k: Schema.model_validate(v)
            for k, v in self.schema.__pydantic_extra__.get("definitions", {}).items()
        }
...
@register_filter
def as_pascal_case(value: str) -> str:
    return value.title().replace("_", "")


@register_filter
def as_snake_case(value: str) -> str:
